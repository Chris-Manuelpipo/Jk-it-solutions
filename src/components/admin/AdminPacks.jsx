...
export default function AdminPacks({ onSave }) {
    ...
    const openNew = () => {
        if (packs.length >= MAX_PACKS) {
            onSave(`Maximum ${MAX_PACKS} packs autorisés. Supprimez-en un d'abord.`);
            return;
        }
    ...
    const handleSave = () => {
        ...
        const cleanForm = { ...form, items: form.items.filter(i => i.trim()), originalPrice: Number(form.originalPrice), promoPrice: Number(form.promoPrice) };
        let updated;
        if (editing === 'new') {
            updated = [...packs, cleanForm];
        } else {
            updated = packs.map(p => p.id === editing ? cleanForm : p);
        }
        setPacks(updated);
        updateContent('packs', updated);
    ...
    const toggleActive = (id) => {
        const updated = packs.map(p => p.id === id ? { ...p, active: !p.active } : p);
        setPacks(updated);
        updateContent('packs', updated);
    };
        ...
        return (
            <div className="admin-card">
                ...
                <div className="admin-form">
                    ...
                    <div className="admin-form-row">
                        <div className="admin-field">
                            <label>Prix original (à barrer)</label>
                            <input name="originalPrice" type="number" value={form.originalPrice} onChange={handleChange} min={0} />
                        </div>
                        <div className="admin-field">
                            <label>Prix promotionnel</label>
                            <input name="promoPrice" type="number" value={form.promoPrice} onChange={handleChange} min={0} />
                        </div>
                        <div className="admin-field">
                            <label>Devise</label>
                            <input name="currency" value={form.currency} onChange={handleChange} placeholder="FCFA" />
                        </div>
                    </div>
                    ...
                    {form.originalPrice > 0 && form.promoPrice > 0 && (
                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '0.75rem 1rem', background: 'var(--light)', borderRadius: 'var(--radius)', fontSize: '0.88rem', flexWrap: 'wrap' }}>
                            ...
                            {discount(form.originalPrice, form.promoPrice) > 0 && (
                                <span style={{ background: '#fee2e2', color: '#dc2626', padding: '0.2rem 0.6rem', borderRadius: '50px', fontWeight: 700 }}>
                                    −{discount(form.originalPrice, form.promoPrice)}%
                                </span>
                            )}
                        </div>
                    )}
                    ...
                    <div className="admin-form-row">
                        <div className="admin-field">
                            <label><i className="fas fa-calendar" /> Date d'expiration (optionnel)</label>
                            <input name="expiresAt" type="date" value={form.expiresAt} onChange={handleChange} />
                            <small style={{ color: 'var(--gray)', fontSize: '0.75rem' }}>Laissez vide si pas de limite</small>
                        </div>
                        <div className="admin-field">
                            <label>Texte du bouton CTA</label>
                            <input name="cta" value={form.cta} onChange={handleChange} placeholder="Ex: Commander ce pack" />
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.88rem', fontWeight: 600 }}>
                            ...
                            ⭐ Pack mis en avant (carte centrale agrandie)
                        </label>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.88rem', fontWeight: 600 }}>
                    ...
                    <div className="admin-field">
                        <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span>Éléments inclus dans le pack</span>
                            <button type="button" onClick={addItem} style={{
                                background: 'var(--primary)', color: 'white', border: 'none', borderRadius: '50px',
                                padding: '0.2rem 0.7rem', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer'
                            }}>
                                + Ajouter
                            </button>
                        </label>
    ...
    return (
        <div className="admin-card">
            <div className="admin-card-header">
                ...
                <button className="btn-admin-save" onClick={openNew} disabled={packs.length >= MAX_PACKS}>
                    <i className="fas fa-plus" /> Ajouter
                </button>
            </div>

            {packs.length >= MAX_PACKS && (
                <div style={{ padding: '0.6rem 1rem', background: '#fef9c3', borderRadius: 'var(--radius)', fontSize: '0.82rem', color: '#92400e', marginBottom: '1rem' }}>
                    <i className="fas fa-info-circle" /> Maximum {MAX_PACKS} packs. Supprimez-en un pour en créer un nouveau.
                </div>
            )}
                        ...
                        <div key={p.id} className="admin-list-item" style={{ opacity: p.active ? 1 : 0.55 }}>
                            ...
                            <div className="admin-list-item-info" style={{ flex: 1 }}>
                                <strong style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    {p.title}
                                    {p.featured && <span style={{ background: '#e0e7ff', color: '#4f46e5', fontSize: '0.65rem', padding: '0.1rem 0.4rem', borderRadius: '50px', fontWeight: 700 }}>⭐ Mis en avant</span>}
                                    {!p.active && <span style={{ background: '#f1f5f9', color: '#94a3b8', fontSize: '0.65rem', padding: '0.1rem 0.4rem', borderRadius: '50px', fontWeight: 700 }}>Masqué</span>}
                                </strong>
                                <span style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
                                    ...
                                    {p.expiresAt && <span style={{ fontSize: '0.75rem', color: '#f59e0b' }}><i className="fas fa-clock" /> Expire le {p.expiresAt}</span>}
                                </span>
                                <span style={{ fontSize: '0.78rem', color: 'var(--gray)' }}>{(p.items || []).length} éléments inclus</span>
                            </div>
                            <div className="admin-list-item-actions">
                                <button
                                    onClick={() => toggleActive(p.id)}
                                    title={p.active ? 'Masquer' : 'Afficher'}
                                    style={{ padding: '0.45rem 0.7rem', fontSize: '0.8rem', borderRadius: 'var(--radius)', border: '1.5px solid var(--gray-light)', background: 'white', cursor: 'pointer', color: p.active ? '#22c55e' : 'var(--gray)' }}
                                >
                                    <i className={`fas ${p.active ? 'fa-eye' : 'fa-eye-slash'}`} />
                                </button>
            ...
            {packs.length === 0 && (
                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--gray)' }}>
                    <i className="fas fa-tags" style={{ fontSize: '2rem', display: 'block', marginBottom: '0.5rem', opacity: 0.3 }} />
                    Aucun pack. Cliquez sur "Ajouter" pour créer votre première offre.
                </div>
            )}
        </div>
    );
